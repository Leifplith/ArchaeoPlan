  ...
  root.traverse(object => {
    ...
    const converted = sourceMaterials.map(source => {
      ...
      return new THREE.MeshBasicMaterial({
        ...
        color: texture
          ? 0xffffff
          : (source?.color ? source.color.clone() : 0xd0d0d0),
        vertexColors: !texture && Boolean(object.geometry?.attributes?.color),
...
function selectModel(model) {
  ...
  if (model && !model.root.userData.locked) {
...
function syncTransformFields() {
  ...
  const position = selectedModel.root.position;
  const rotation = selectedModel.root.rotation;

  $('posX').value = position.x.toFixed(3);
  $('posY').value = position.y.toFixed(3);
  $('posZ').value = position.z.toFixed(3);
  $('rotX').value = THREE.MathUtils.radToDeg(rotation.x).toFixed(2);
  $('rotY').value = THREE.MathUtils.radToDeg(rotation.y).toFixed(2);
  $('rotZ').value = THREE.MathUtils.radToDeg(rotation.z).toFixed(2);
...
function switchCamera(useOrthographic) {
  ...
  if (selectedModel && !selectedModel.root.userData.locked) {
...
async function loadFiles(fileList) {
  ...
  const objectUrls = new Map(
    files.map(file => [file.name.toLowerCase(), URL.createObjectURL(file)])
  );
  ...
  if (mainFiles.length === 0) {
    setStatus('Der blev ikke fundet en GLB-, GLTF-, OBJ- eller PLY-fil.');
    return;
  }
    ...
    try {
      ...
      if (extension === 'obj') {
        ...
        if (materialReference) {
          const materialName = materialReference[1].trim().toLowerCase();
...
async function exportPng() {
  ...
  perspectiveCamera.aspect = previousPerspectiveAspect;
  Object.assign(orthographicCamera, previousOrtho);
  camera.updateProjectionMatrix();
...
$('lockButton').addEventListener('click', () => {
  ...
  if (!selectedModel.root.userData.locked) {
    transform.attach(selectedModel.root);
  }
...
for (const button of document.querySelectorAll('[data-view]')) {
  button.addEventListener('click', () => setStandardView(button.dataset.view));
}
